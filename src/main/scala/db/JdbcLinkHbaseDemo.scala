...
object JdbcLinkHbaseDemo {
  def main(args: Array[String]): Unit = {
    val sparkConf = new SparkConf().setMaster("spark://qiwr1:7077")
      .setAppName("reduce")
    val sc = new SparkContext(sparkConf)

    val tablename = "t_user_info"
    val conf = HBaseConfiguration.create()
    //设置zooKeeper集群地址，也可以通过将hbase-site.xml导入classpath，但是建议在程序里这样设置
    conf.set("hbase.zookeeper.quorum", "qiwr1:2181,qiwr2:2181,qiwr3:2181")
    //设置zookeeper连接端口，默认2181
    conf.set("hbase.zookeeper.property.clientPort", "2181")
    conf.set(TableInputFormat.INPUT_TABLE, tablename)
    conf.set(TableInputFormat.SCAN_COLUMNS, "t_user_info:user_001")
    ...
    hBaseRDD.foreach { case (_, result) => {
      ...
      //通过列族和列名获取列
      val typenames = Bytes.toString(result.getValue("t_user_info".getBytes, "type".getBytes))
    ...
    //设置zooKeeper集群地址，也可以通过将hbase-site.xml导入classpath，但是建议在程序里这样设置
    resultConf.set("hbase.zookeeper.quorum", "qiwr1:2181,qiwr2:2181,qiwr3:2181")
    //设置zookeeper连接端口，默认2181
